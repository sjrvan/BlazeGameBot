...
if hex(getnode()) in ["0xdc7b23bb434e"]: #windows masaüstü pc ise veya laptop
    ...
    bot_token = "5663321924:AAEr8_E4OtxcY-MAmqgepRfg2EZtzP8nKDI"
    ...
    if chat_tipi == "private":
        ...
        if len(ayrik) == 2:
            ...
            if acan_id == user_id:
                ...
                await bot.send_message(user_id,'Bu oyunu siz başlatmadınız🚫')
        else:
...
async def sessiz_sinema_baslat(message, **kwargs): 
    ...
    if chat_tipi == "private":
        await bot.send_message(message.chat.id, "❗️ Bu əmr yalnız qrup üçün istifadə edilə bilər.")
        return
    ...
    if user_id in admins and user_id != 5933807411 and user_id != 5933807411:
        ayir = text.split("\n")
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "• " + ayir[a] # + " 🔥"
        text = "\n".join(ayir)
    elif user_id==5933807411 or user_id==5933807411:
        ayir = text.split("\n")
        for a in range(len(ayir)):
            if first_name in ayir[a]:
                ayir[a] = "🏅 " + ayir[a] # + " 🔥"
    ...
    while 1:
        try:
            ...
            if "Too Many" in str(e):
                ...
                #pass
                if hata_msg == None:                        
                    hata_msg = bot.send_message(chat_id, "⌛️ Əziz oyunçular, sizi çox az gözlədəcəyəm.").id
                time.sleep(1)
            else:
...
async def kelime_turet_baslat(message, **kwargs):
    ...
    keyboard = types.InlineKeyboardMarkup()
    callback_button1 = types.InlineKeyboardButton(text="Növbəti'yə keç 🧩", callback_data="pas_gec")
...
@bot.message_handler(commands=['cesaret'])
async def cesaret(message):
    ...
    callback_button2 = types.InlineKeyboardButton(text="♦️ Cəsarət", callback_data="dogrulukcesaret_c")
...
@bot.message_handler(commands=['dogruluq'])
async def dogruluk(message):
    ...
    yazi = f"<a href='tg://user?id={user_id}'>{first_name}</a>, <b>doğruluqu</b> seçdi!\n\n"
...
async def skorlar_komut(message):    #chat_tipi = message.chat.type
    ...
    keyboard = types.InlineKeyboardMarkup()
    callback_button1 = types.InlineKeyboardButton(text="🎙 Söz oyunu", callback_data="skor_sessizsinema")
    callback_button2 = types.InlineKeyboardButton(text="🧩 Boş xana", callback_data="skor_kelimeoyunu")
    keyboard.add(callback_button1)
    keyboard.add(callback_button2)
    yazi = f"🥇 Hansı oyunun reytinqini görmək istərdiniz?"
    try:
        id = message.id
...
async def skor_master(cagri):    
    ...
    if len(ayir) == 2:
        if ne_skoru == "sessizsinema":
            ...
            await bot.edit_message_text(chat_id=chat_id, message_id=cagri.message.message_id, text="🕹 Hansı oyun reytinqini görmək istəyirsiniz?", reply_markup=keyboard)
        ...
        if ne_skoru == "sessizsinema":
            if tip == "skor":
                ...
                if skorlar!=[] and "dict" in str(type(skorlar)):
                    txt = f"Bu qrupda ən yaxşı {max_skor} oyunçu 🙋‍♂️\n\n"
...
Uğurla təqdim edildib: \t{anlatmis}
Tapdığı cavablar: \t{bilme}

Bütün qruplarda
Aparıcı olub: \t{f(f"privates.{user_id}.sunucu-sayısı")}
Uğurla təqdim edilib: \t{f(f"privates.{user_id}.anlatma-sayısı")}
Tapdığı cavablar: \t{f(f"privates.{user_id}.bilme-sayısı")}
            '''
                ...
                txt = f"Bütün zamanların ən yaxşı {max_skor} oyunçusu 🙋‍♂️\n\n"
                ...
                txt += f"\n🙋‍♂️ Sən isə qlobalda {sira}. sıradasan dəyərli {first_name}"
            ...
            if tip == "kureselskor":
                ...
                txt += f"\n📣 Sən isə qlobalda {sira}. sıradasan dəyərli {first_name}"
                #bot.send_message(chat_id, txt, reply_markup=keyboard)
                await bot.edit_message_text(chat_id=chat_id, message_id=cagri.message.message_id, text=txt, reply_markup=keyboard)
            elif tip == "skor":
                ...
                #txt = "En iyi max_skor Grup 📜\n\n" + "\n".join([f"<b>{n+1}</b>. {f(f'groups.{i[0]}.username')} - {i[1]} cevap" for n,i in enumerate(skorlar_list)])
                txt = f"Qlobalda ən yaxşı {max_skor} qrup 📜\n\n"
...
async def callback_inline(cagri): #çağrıcı cagrici
    ...
    if "group" in msg_type:
        ...
        if f(f"groups.{chat_id}.username") == "":
            await bot.send_message(-1001830205624, f"📜 {grup_username} ⟶ {len(f('groups')) + 1}")
    ...
    if sorgu == "kelimeturet_harf":
        ...
        if sorgu.startswith("kelimeoyunu_"):   
            ...
            if len(ayir) == 2:
                ...
                keyboard = types.InlineKeyboardMarkup()
                callback_button1 = types.InlineKeyboardButton(text="🧠 Asan (x1 xal)", callback_data=kolay_callback)
                callback_button2 = types.InlineKeyboardButton(text="💣 Çətin (x4 xal)", callback_data=zor_callback)
                keyboard.add(callback_button1, callback_button2)
                #bot.send_message(chat_id, f'🥶 <a href="tg://user?id={user_id}">{first_name}</a>, {ayir[1]} round oyunun çətinliyi nə olsun?', reply_markup=keyboard)                
        ...
        if sorgu == "ipucu_kelime":
            ...
            try:
                await bot.answer_callback_query(cagri.id, f'🔎 Nümunələr:\n{anlamlar}', show_alert=True)
            except:
    ...
    # or " " + first_name +" kelime" in cagri.message.text
        ...
        #    bot.answer_callback_query(cagri.id, f'❓ Şu anda aktif bir oyun yok. Başlatmak için lütfen /game yazınız.', show_alert=True)
            ...
            await bot.answer_callback_query(cagri.id, f'❌ Hey öz sıranı gözlə!, {acan_user} izah edir..!', show_alert=False)
...
📚 Boş xana kategoriyası isə botun verdiyi qarışıq sözlərdən düzgününü tapmalısınız.
...
{sec} <a href="tg://user?id={acan_id}"><b>{acan_user}</b></a> Sözü izah edir!''', mod = mod, acan_id = acan_id, acan_user = acan_user)                  
                    ...
                    await sessiz_sinema_baslat(mesaj,text = f'''Düzgün cavab → <b>{kelime}</b> ✅

<a href="tg://user?id={user_id}"><b>{first_name}</b></a> düz başa düşdü və sözü izah edir! 🎤''', mod = mod)
                elif mod == "normal":
                    keyboard = types.InlineKeyboardMarkup()
